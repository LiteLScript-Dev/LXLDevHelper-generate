...
class Generateble {
    ...
    fun run(jsonStr: String) {
        ...
        val jsCode = JsGenerate("").makeClass(ClassData.classCache, ClassData.classInfo)
        File("$jsUrl/Class/").mkdirs()
        File("$jsUrl/Class/${ClassData.classInfo.className}.js").writeText(jsCode, Charsets.UTF_8)
        log.info("Writing JS File to $jsUrl/Class/${ClassData.classInfo.className}.js")

        val temp = "\n/// <reference path=\"./Class/${ClassData.classInfo.className}.js\" />"
        File("${jsUrl}Api.js").appendText(temp, Charsets.UTF_8)
        log.info("Writing JS Api File to ${jsUrl}Api.js")
